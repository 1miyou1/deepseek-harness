...
export function hasUserTask(messages) {
 ...
 */
export function routeAgentStep(messages, routes) {
    const task = messages
        ...
        .join('\n');
    if (HIGH_RISK.test(task))
        return { tier: 'sol', selection: routes.sol, reason: 'high-risk' };
    if (COMPLEX.test(task))
        return { tier: 'sol', selection: routes.sol, reason: 'complex' };
    if (LIGHTWEIGHT.test(task))
        return { tier: 'luna', selection: routes.luna, reason: 'lightweight' };
